...
contract Bidding is AccessControl, Pausable {
    ...
    function depositPrize(address _nftAddress, uint tokenId)
        external
    ...
    function removeMember(bytes32 role, address account)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function bid(uint amount)
        external
        onlyOnRunningAuction
        onlyAfterPrizeDeposited
    {
        require(tx.origin == msg.sender, "Contract not allowed to bid");
        require(
            amount > highestBid + minBidAmount,
            "Bid must be higher than highest bidder"
        );
